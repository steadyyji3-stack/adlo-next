      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
        <div className="mb-8 border-b border-slate-200 pb-7">
          <div className="mb-3 flex items-center gap-2 text-[#0F6E56]">
            ...
            <span className="text-xs font-extrabold uppercase">Review Workspace</span>
          </div>
          <h1 className="text-3xl font-extrabold text-slate-950 sm:text-4xl">評論回覆工作區</h1>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">
            貼入公開評論，取得三種台灣口吻的回覆草稿；選擇、編輯、儲存與複製都在同一頁完成。
          </p>
        </div>

        {!customer ? (
          <Gate title="請先登入客戶後台" body="請使用訂閱 email 的 magic link 登入後再開始。" href="/customer/login?next=/customer/tools/review-workspace" action="前往登入" />
        ) : !canUse ? (
          <Gate title="此功能需要有效訂閱" body="啟用或恢復訂閱後，每日可產生 20 則評論回覆草稿。" href="/customer/billing" action="查看訂閱" />
        ) : (
          <ReviewWorkspace defaultIndustry={customer.industry ?? ''} />
        )}
      </main>
    </div>
  );
