...
import {
  MapPin, Search, PenLine, MousePointerClick,
...
const advancedModules = [
  {
    id: 'ai-management', num: '05', icon: Bot, title: 'AI 代管協助 + 企業 Agent',
    desc: '建立專屬 AI Agent、自動化工作流程、客服機器人與內容生成管道。讓 AI 成為你團隊的一部分，24 小時不間斷運作。',
    items: ['企業 AI Agent 建立', '客服自動化 Chatbot', 'n8n 工作流程自動化', 'AI 系統維護與優化'],
    href: '/services/ai-management',
    isNew: true,
  },
  {
    id: 'market-research', num: '06', icon: BarChart2, title: '產品數位市調',
    desc: '不問 1,000 人也能拿到市調結果。搜尋行為數據 + AI 合成，數天內產出偏好分布報告，讓每個決策有數據支撐。',
    items: ['AI 模擬市調報告', '搜尋偏好分佈分析', '社群聲量情緒分析', '競爭對手廣告驗證'],
...
const serviceJsonLd = {
  ...
  '@type': 'ItemList',
  name: 'adlo 六大服務模組',
  itemListElement: allModules.map((m, i) => ({
    '@type': 'ListItem',
    position: i + 1,
    item: {
      '@type': 'Service',
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        title="六大服務模組"
        description="從在地曝光、SEO、廣告到 AI 自動化，靈活組合適合你的行銷方案"
      />
      ...
      <section className="py-20 px-6 md:px-8 bg-slate-50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-slate-500 text-sm">可單獨使用，也可自由組合搭配</p>
            <div className="w-16 h-0.5 bg-[#1D9E75] mx-auto mt-4" />
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {allModules.map(({ id, num, icon: MIcon, title, desc, items, href, isNew }) => (
              <Card key={id} id={id} className="card-hover border-slate-100 scroll-mt-20 group flex flex-col">
                <CardHeader className="pb-0 p-7">
                  <div className="flex items-start justify-between mb-4">
                    <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center group-hover:bg-[#1D9E75] transition-colors">
                      <MIcon className="w-5 h-5 text-slate-600 group-hover:text-white transition-colors" />
                    </div>
                    {isNew && (
                      <span className="text-[10px] font-extrabold bg-[#1D9E75] text-white px-2 py-0.5 rounded-full leading-none">NEW</span>
                    )}
                  </div>
                  <Badge variant="outline" className="text-[#0F6E56] border-[#1D9E75]/30 bg-[#E1F5EE] text-xs font-bold tracking-widest w-fit mb-2">
                    MODULE {num}
                  </Badge>
                  <h3 className="text-lg font-bold text-slate-800 leading-snug">{title}</h3>
                </CardHeader>
                <CardContent className="p-7 pt-3 flex flex-col flex-1">
                  ...
                  {href && (
                    <Link href={href}
                      className="inline-flex items-center gap-1.5 mt-6 text-sm font-semibold text-[#1D9E75] hover:text-[#0F6E56] transition-colors">
      ...
      {/* CTA */}
      <section className="py-24 px-6 md:px-8 bg-[#E1F5EE] text-center">
