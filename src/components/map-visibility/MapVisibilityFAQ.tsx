...
import { MAP_VISIBILITY_FAQS } from './faq-data';

export default function MapVisibilityFAQ() {
    ...
    <section className="bg-white py-16 md:py-24">
      <div className="max-w-3xl mx-auto px-6 md:px-8">
        ...
        <div className="space-y-3">
          {MAP_VISIBILITY_FAQS.map((faq, idx) => {
            const isOpen = open === idx;
            const panelId = `map-visibility-faq-panel-${idx}`;
            const buttonId = `map-visibility-faq-button-${idx}`;
            return (
              <div
                key={idx}
